...
async def run_test():
    ...
    try:
        ...
        # -> Open the auth overlay by clicking the 'Se connecter' button so the login form appears.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div/div/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill in the email field with admintest@test.com, then fill the password and submit the login form.
        frame = context.pages[-1]
        # Input text
